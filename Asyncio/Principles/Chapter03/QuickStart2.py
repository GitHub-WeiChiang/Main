...
if __name__ == '__main__':
    ...
    # In fact, anywhere you call it,
    # get_event_loop() will give you the same loop instance each time,
    # as long as you’re using only a single thread.
    # If you’re inside an async def function,
    # you should call asyncio.get_running_loop() instead.
    loop = asyncio.get_event_loop()

    # In this case, the specific call is loop.create_task(main()).
    ...
    # When the “main” part of the program unblocks,
    # either due to a process signal being received or the loop being stopped by some code calling loop.stop(),
    # the code after run_until_complete() will run.
    # The standard idiom as shown here is to gather the still-pending tasks, cancel them,
    # and then use loop.run_until_complete() again until those tasks are done.
    # gather() is the method for doing the gathering.
    # Note that asyncio.run() will do all the cancelling, gathering, and waiting for pending tasks to finish up.
    group = asyncio.gather(*pending, return_exceptions=True)
    loop.run_until_complete(group)

    # loop.close() is usually the final action,
    # it must be called on a stopped loop,
